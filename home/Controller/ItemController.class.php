...
class ItemController extends ArController
{
    ...
    public function sendLogAction()
    {
        ...
        if (!isset($post['id'])) {
            ...
            $result = U_item_develop_logModel::model()->sendLog($post);
        }
    ...
    //查看项目日志
    public function listItemLogAction()
    {
        $post = arRequest();

        if (!isset($post['id'])) {
            $this->showJsonError('没有传入项目id',1355);
        } else {
            $result = U_item_develop_logModel::model()->listItemLog($post);
        }

        if ($result) {
            $this->showJson($result);
        } else {
            $this->showJsonError('查看失败！', 1356);
        }
    }
